  ...
  #define w4d(i3, i2, i1, i0) weights[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
      ...
      if (n < B && m < M && h < H_out && w < W_out) {
        ...
        for (c = 0; c < C; c++) {
          for (p = 0; p < K; p++) {
            for (q = 0; q < K; q++) {
              acc += x4d(n, c, h + p, w + q) * w4d(m, c, p, q);
            }
          }
        }
        y4d(n, m, h, w) = acc;
 ...
 */
    ...
    void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
    {
      ...
      weightCopy = w.dptr_;
      float hostWeight[14112];
      cudaMemcpy(hostWeight, weightCopy, sizeof(float)*M*C*K*K, cudaMemcpyDeviceToHost);
      cudaMemcpyToSymbol(weights, hostWeight, sizeof(float)*M*C*K*K, 0, cudaMemcpyHostToDevice);

      /* float* X_unrolled;
         cudaMalloc((void **) &X_unrolled, H_unroll * W_unroll * sizeof(float));

         for(int n = 0; n < B; n++){
         unroll_kernel<<<ceil((float)(C*H_out*W_out/CUDA_MAX_NUM_THREADS)), CUDA_MAX_NUM_THREADS>>>(n, C, H, W, K, x.dptr_, X_unrolled);
         cudaDeviceSynchronize();
         matrixMultiplyShared<<<dim3(ceil((float)W_unroll/TILE_WIDTH), ceil((float)M/TILE_WIDTH), 1), dim3(TILE_WIDTH, TILE_WIDTH, 1)>>>(w.dptr_, X_unrolled, &y.dptr_[n*M*H_out*W_out], M, H_unroll, H_unroll, W_unroll, M, W_unroll);
         cudaDeviceSynchronize();
         }*/
